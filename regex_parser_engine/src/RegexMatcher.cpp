...
RegexMatcher::RegexMatcher(const char *regex, Alphabet *alphabet) {
	if(!alphabet)
...
bool RegexMatcher::match(const char *input) {
	const char *c = input;
	int curState = 0;
	m_dfa->reset();
	while(*c) {
...
bool RegexMatcher::matchPartially(const char *input, int *pMatchLength) {
	const char *c = input;
	int lastState;
	int curState;
...
bool RegexMatcher::contains(const char *input, int *pStartPos, int *pEndPos) {
	const char *c = input;
	int lastState;
	int curState;
